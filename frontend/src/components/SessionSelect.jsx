...
const SessionSelect = ({ user }) => {
  ...
  const cards = [
    ...
    { key: 'simulacao', title: 'Simulator', icon: Calculator, to: '/simulacao' },
    { key: 'catalogo-pro', title: 'Catálogo PRO', icon: Calculator, to: '/catalogo-pro' },
  ];
  
  const subtitleFor = (key) => {
    if (key === 'pricing-dashboard') return 'Histórico de preços e margens';
    if (key === 'pre-vendas') return 'Novos projetos e preços';
    if (key === 'cs') return 'Calendário e controle de reajuste';
    if (key === 'simulacao') return 'Simulador de preços e margens';
    if (key === 'catalogo-pro') return 'Em desenvolvimento';
    return 'Acessar';
  };

  const titleClassFor = (key) => {
    if (key === 'pre-vendas') return 'text-lg';
    return 'text-xl';
  };

  const subtitleClassFor = () => 'text-sm whitespace-nowrap';
          ...
          {cards.map(({ key, title, icon: Icon, to }) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="p-2.5 rounded-lg bg-gray-100 dark:bg-gray-900 text-blue-600 dark:text-blue-400 group-hover:bg-blue-50 dark:group-hover:bg-blue-900/20 transition-colors">
                  <Icon size={24} />
                </div>
                <div>
                  <h2 className={`${titleClassFor(key)} font-semibold text-gray-900 dark:text-white mb-1`}>{title}</h2>
                  <p className={`${subtitleClassFor()} text-gray-500 dark:text-gray-400`}>{subtitleFor(key)}</p>
                </div>
              </div>
            </button>
          ))}
